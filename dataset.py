...
#ReCAM加载多类别标签列表from .npy文件[1,1,0,0]
def recam_load_img_label_list_from_npy(img_name_list, dataset):
    cls_labels_dict = np.load(f'metadata/{dataset}/cls_labels_recam.npy', allow_pickle=True).item()
    return [cls_labels_dict[img_name] for img_name in img_name_list]
...
class ClassificationDatasetWithSaliency(ImageDataset):
    ...
    def __init__(self, dataset, img_id_list_file, img_root, saliency_root=None, crop_size=224, resize_size=(256, 512)):
        ...
        self.label_list = load_img_label_list_from_npy(self.img_id_list, dataset)
        #ReCAM加载类别标签
        self.recam_label_list = recam_load_img_label_list_from_npy(self.img_id_list, dataset)

    def __getitem__(self, idx):
        img_id = self.img_id_list[idx]
        img = PIL.Image.open(os.path.join(self.img_root, img_id + '.jpg')).convert("RGB")
        label = torch.from_numpy(self.label_list[idx])
        recam_label = torch.from_numpy(self.recam_label_list[idx])

        # 获取文件名前缀作为类别标识 如果图像为无缺陷图像（文件名前缀为0） 则其显著性图自动生成与原图尺寸相等的黑色RGB图像
        ...
        if(class_prefix>0):
            ...
            return img_id, img, saliency, label, recam_label
        else:
            h,w = img.size
            saliency = PIL.Image.new("RGB",(h,w))
            img, saliency = self.transform_with_mask(img, saliency)
            return img_id, img, saliency, label, recam_label
